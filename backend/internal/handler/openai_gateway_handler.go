...
func (h *OpenAIGatewayHandler) validateFunctionCallOutputRequest(c *gin.Context, body []byte, reqLog *zap.Logger) bool {
	...
	if validation.HasFunctionCallOutputMissingCallID {
		...
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "function_call_output requires call_id on HTTP requests; include previous_response_id or matching tool-call context when continuing a response")
		return false
	}
	...
	h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "function_call_output requires item_reference ids matching each call_id on HTTP requests, unless previous_response_id provides the response context")
	return false
}
